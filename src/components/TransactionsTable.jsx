...
const TransactionsTable = ({ 
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mt-8 overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
          ...
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all placeholder:text-gray-400"
            />
          </div>
      ...
      {isAdding && isAdmin && (
        <form onSubmit={handleSubmit} className="mb-8 p-4 bg-gray-50 rounded-xl border border-gray-200 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Date</label>
              <input required type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full p-2 border rounded-lg focus:ring-2 ring-blue-500/20 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Category</label>
              <input required type="text" placeholder="e.g. Shopping" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-2 border rounded-lg focus:ring-2 ring-blue-500/20 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Type</label>
              <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full p-2 border rounded-lg focus:ring-2 ring-blue-500/20 outline-none">
            ...
            <div>
              ...
              <input required type="number" step="0.01" placeholder="0.00" value={formData.amount} onChange={e => setFormData({...formData, amount: parseFloat(e.target.value)})} className="w-full p-2 border rounded-lg focus:ring-2 ring-blue-500/20 outline-none" />
            </div>
          </div>
            ...
            {filtered.length > 0 ? (
              filtered.map((t) => (
                <tr key={t.id} className="group hover:bg-gray-50/50 transition-colors">
                  <td className="py-4 text-sm font-medium text-gray-600 border-none">{t.date}</td>
                  <td className="py-4 text-sm font-bold text-gray-800 border-none">{t.category}</td>
                  ...
                  <td className={cn(
                    "py-4 text-sm font-extrabold border-none",
